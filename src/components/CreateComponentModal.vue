...
const handleConfirm = () => {
  if (componentName.value.trim() && !props.loading) {
    emit('confirm', componentName.value.trim())
  }
}
  ...
  <div v-if="isOpen" 
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
      ...
      <div class="px-6 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
        <div>
          <h2 class="text-lg font-bold text-slate-800">Criar Novo Componente</h2>
          <p class="text-[11px] text-slate-500 mt-0.5">Transforme o elemento selecionado em um componente reutilizável.</p>
        </div>
        <button @click="close" 
      ...
      <!-- Body -->
      <div class="p-6">
        <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Nome do Componente</label>
        <div class="relative flex items-center">
          <input 
            ref="inputRef"
            v-model="componentName"
            type="text"
            placeholder="ex: cards/user-profile"
            class="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 font-medium focus:border-indigo-500 focus:bg-white focus:outline-none transition-all rounded-xl text-sm"
            @keydown.enter="handleConfirm"
            :disabled="loading"
          />
        </div>
        <p class="mt-2 text-[10px] text-slate-400">Dica: Use barras para organizar em pastas (ex: cards/card-v1).</p>
      </div>
