...
<template>
  <div class="flex flex-col gap-2">
    <VisualSectionHeader label="Margin">
      <template #actions>
        <button 
          v-if="!isLinked"
    ...
    <div v-if="isLinked" class="flex flex-col gap-2">
      ...
      >
        <template #icon>
           <svg class="w-3.5 h-3.5 text-blue-500 opacity-60" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="20" stroke-dasharray="2 2" /><rect x="6" y="6" width="12" height="12" /></svg>
        </template>
      </VisualInput>
    </div>
    ...
    <div v-else class="grid grid-cols-2 gap-x-2 gap-y-2 p-1 bg-gray-50/50 rounded border border-gray-100">
      <div v-for="s in [{id:'t',label:'Top'},{id:'r',label:'Right'},{id:'b',label:'Bottom'},{id:'l',label:'Left'}]" :key="s.id" class="flex flex-col gap-1">
        ...
        <VisualInput 
          ...
          :units="units" :keywords="keywords" allow-negative compact
