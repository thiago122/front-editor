...
const { previewWidth, previewUnit, startPreviewResizeRight, startPreviewResizeLeft } = usePreviewResize(startResize)
...
<template>
  <div class="flex flex-col grow shrink-0 h-full max-h-full overflow-hidden">
    ...
    <div class="flex grow overflow-hidden bg-white"
      ...
      <EditorSidebar v-model:active-explorer="activeExplorer" @open-import="isImportModalOpen = true" @save="saveNow"
        @download-css="downloadCss" @download-html="downloadHtml" />

      <!-- col-layer: HTML Explorer -->
      <template v-if="activeExplorer === 'html'">
        <AsidePanel id="col-layer" title="LayerExplorer"
          :style="{ width: layerWidth + 'px', minWidth: '160px', maxWidth: '520px' }"
          style2="position: relative; z-index: var(--z-panel)">
          <ASTExplorer :ast="EditorStore.ast" :selectedNodeId="EditorStore.selectedNodeId"
            :openPath="EditorStore.openPath" />
        </AsidePanel>
        <!-- Handle de resize: col-layer -->
        <div class="w-1 shrink-0 cursor-col-resize bg-transparent hover:bg-blue-400/40 transition-colors z-50"
          title="Arrastar para redimensionar" @mousedown="startLayerResize" />
      </template>

      <!-- col-components: Component Explorer -->
      <template v-if="activeExplorer === 'components'">
        <AsidePanel id="col-components" title="Componentes"
          :style="{ width: layerWidth + 'px', minWidth: '160px', maxWidth: '520px' }"
          style2="position: relative; z-index: var(--z-panel)">
          <ComponentExplorer />
        </AsidePanel>
      ...
      <div class="flex flex-col h-full w-full overflow-hidden bg-[#f1f1f1]" id="col-main">
        ...
        </div>{{ previewWidth }} {{ previewUnit }}
        <div ref="previewContainerEl" class="grow shrink-0 overflow-auto flex justify-center overflow-hidden"
          style="position: relative; background-color: #f1f1f1">
          <div class="relative shrink-0 flex items-stretch z-0 shadow-lg" :style="{
        ...
        <FloatingWindow :show="EditorStore.htmlEditor.show" title="Editor HTML" :initialX="EditorStore.htmlEditor.x"
          :initialY="EditorStore.htmlEditor.y" :initialWidth="800" :initialHeight="500" :closeOnClickOutside="false"
          ...
          <template #header-left>
            ...
            <span v-if="EditorStore.htmlEditor.targetId"
              class="text-[10px] text-gray-500 font-mono truncate max-w-[200px]">
              ID: {{ EditorStore.htmlEditor.targetId }}
            </span>
          </template>

          <CodeEditor ref="codeEditorHtmlRef" mode="html" :targetId="EditorStore.htmlEditor.targetId"
            :show="EditorStore.htmlEditor.show" :hideHeader="true" @close="EditorStore.htmlEditor.show = false" />
        </FloatingWindow>

        <!-- Code Editor CSS Flutuante -->
        <FloatingWindow :show="EditorStore.cssFileEditor.show" title="Editor CSS"
          :initialX="EditorStore.cssFileEditor.x" :initialY="EditorStore.cssFileEditor.y" :initialWidth="800"
          :initialHeight="500" :closeOnClickOutside="false" @close="EditorStore.cssFileEditor.show = false">
          <template #header-left>
            <div
            ...
            <div class="h-3 w-[1px] bg-gray-700 mx-1"></div>
            <span v-if="EditorStore.cssFileEditor.targetId"
              class="text-[10px] text-gray-500 font-mono truncate max-w-[200px]">
          ...
          <template #header-right>
            <div v-if="codeEditorCssRef?.isBulkMode" class="flex items-center">
              <button @click="codeEditorCssRef.handleSave"
                :disabled="!codeEditorCssRef.hasUnsavedChanges || codeEditorCssRef.isSaving"
                class="flex items-center gap-1.5 px-3 h-6 rounded text-[10px] font-bold transition-all border" :class="codeEditorCssRef.hasUnsavedChanges
                  ...
                  ">
                <svg v-if="codeEditorCssRef.isSaving" class="animate-spin h-3 w-3 text-white" fill="none"
                  viewBox="0 0 24 24">
          ...
          <CodeEditor ref="codeEditorCssRef" mode="css" :targetId="EditorStore.cssFileEditor.targetId"
            :show="EditorStore.cssFileEditor.show" :hideHeader="true" @close="EditorStore.cssFileEditor.show = false" />
        </FloatingWindow>
      </div>
      ...
      <template v-if="EditorStore.showCssExplorer && !EditorStore.isDesignerMode">
        ...
        <div
          ...
          title="Arrastar para redimensionar" @mousedown="startCssRightResize" />
        <div id="col-css-right" :style="{ width: cssWidth + 'px', minWidth: '220px', maxWidth: '680px' }">
          <CssExplorer />
        </div>
      </template>
      ...
      <div class="w-1 shrink-0 cursor-col-resize bg-transparent hover:bg-blue-400/40 transition-colors z-50"
        title="Arrastar para redimensionar" @mousedown="startInspectorResize" />

      <!-- col-panel-inspector: Inspector de CSS/HTML -->
      <div id="col-panel-inspector" class="grow-0 shrink-0 border-l border-gray-200 relative"
    ...
    <FloatingWindow :show="EditorStore.pixelPerfectEditor.show" title="Pixel Perfect" theme="light"
      :initialX="EditorStore.pixelPerfectEditor.x" :initialY="EditorStore.pixelPerfectEditor.y" :initialWidth="240"
      :initialHeight="400" :closable="true" :closeOnClickOutside="false"
    ...
    <FloatingWindow :show="EditorStore.variablesPanel.show" title="Design Tokens" theme="light"
      :initialX="EditorStore.variablesPanel.x" :initialY="EditorStore.variablesPanel.y" :initialWidth="300"
      :initialHeight="450" :closable="true" :closeOnClickOutside="false"
      @close="EditorStore.variablesPanel.show = false" @move="
        (pos) => {
